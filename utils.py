...
def get_weights(state_dict):
    return OrderedDict([(k, v.cpu().numpy()) for k, v in state_dict.items()])

def to_torch(state_dict):
    return OrderedDict([(k, torch.FloatTensor(v)) for k, v in state_dict.items()])
...
for i in range(2):
    ...
    while not true_done:
        ...
        memory.append(obs)
        print(torch.FloatTensor(obs).size())
        ...
        #     plt.pause(0.1)
        # print(get_weights(net.state_dict()))
        # a = get_weights(net.state_dict())
        # b = to_torch(a)
