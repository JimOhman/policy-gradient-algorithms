...
def get_algo(net, optimizer, env, args):
    if args['algorithm'] == 'VPG':
        algo = VPG(net=net, optimizer=optimizer, env=env, args=args)
    else:
        raise NotImplementedError
    return algo


def train(algo, args, logr=None):
    verbose = args['verbose']
    total_batches = args['total_batches']
    render = args['render_env']
    print("\nStarting training towards {} batch updates. \n".format(total_batches))
    for batch_num in range(total_batches):
        batch_loss, batch_rets, batch_lens = algo.train_one_batch(render=render)
...
def init(args):
    ...
    algo = get_algo(net=net, optimizer=optimizer, env=env, args=args)
    print("3. Assembled algorithm: {}".format(args['algorithm']))
    time.sleep(1.)
    return env, net, optimizer, algo
...
def evaluate(algo, args):
    ...
    for episode_num in range(args['eval_episodes']):
        ep_rews, ep_obs, _ = algo.run_one_episode(render=render)
...
if __name__ == '__main__':
    ...
    env, net, optimizer, algo = init(args)

    if args['mode'] == 'train':
        train(algo=algo, args=args, logr=logr)
